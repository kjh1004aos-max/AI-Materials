...
def save_checkpoint(ckpt_dir, state, epoch, ema_model=None):
    saved_state = {'epoch': epoch, 'model': state['model'].state_dict()}
...
def create_model_name_and_dir(args):
    name = f'conditional-bs={args.batch_size}--lr={args.learning_rate:.4f}-ch_mult={args.ch_mult}-attn_res={args.attn_resolution}-unet_ch={args.unet_channels}'
    if args.use_stft:
...
def get_x_and_mask(args, data):
    if args.dataset in ['climate', 'physionet']:
        ...
        mask_ts[mask_ts <= args.mask_rate] = 0
        mask_ts[mask_ts > args.mask_rate] = 1
    return (mask_ts, x_ts)