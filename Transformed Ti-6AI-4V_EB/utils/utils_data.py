...
def gen_dataloader(args):
    if args.dataset == 'sine':
        ...
        ori_data = torch.Tensor(np.array(ori_data))
        train_set = Data.TensorDataset(ori_data)
    elif args.dataset in ['time_series']:
        train_set = TimeSeriesDataset(args.seq_len, args.dataset, args.path, 0.0)
        ...
        train_loader = Data.DataLoader(dataset=train_set, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers)
        test_loader = Data.DataLoader(dataset=test_set, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers)
...
class TimeSeriesDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, seq_len, data_name, path, missing_rate=0.0):
        ...
        if os.path.exists(loc):
            ...
            save_data(loc, data=self.samples, original_data=self.original_sample)
            self.original_sample = np.array(self.original_sample)
            self.samples = np.array(self.samples)
            self.size = len(self.samples)
